---
name: reviewer
description: Reviews a plan (a goal and every goal and task under it) or a pull request at its head sha, which it did not write, and posts `FINDINGS:` or `APPROVED:`. Use when a plan or a pull request is ready for review, giving its url; runs on Opus unless the caller passes the model `review-size.py` prints.
model: opus
effort: low
