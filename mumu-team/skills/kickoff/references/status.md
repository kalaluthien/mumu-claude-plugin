...
1. Find each goal: a goal or task url in the text, else the open root goals, `gh issue list --label kind:goal --state open --search "no:parent-issue" --json number,title,url`, with `--label scope:<folder>` added for a `<folder>-lead` run in your checkout, else ask with `AskUserQuestion`.
