...
1. Find each goal: a goal or task url in the text, else the open root goals, `gh issue list --label goal --state open --search "no:parent-issue" --json number,title,url` run in your checkout, else ask with `AskUserQuestion`.
