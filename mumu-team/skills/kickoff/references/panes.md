...
| `start-lead` | another project's leader, in its folder and not a worktree: `herdr tab create --cwd <folder> --label <project>-lead` gives the pane; then `herdr agent start <project>-lead --kind claude --pane <pane> -- --name <project>-lead --model opus --effort medium`, and `prompt` it |
