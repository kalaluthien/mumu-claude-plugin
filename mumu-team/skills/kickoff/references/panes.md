...
| `start` | `worker-start.py <checkout> <topic> <effort> <task-url> [--continue] [--leader <your address>] [--owner-effort]`: `<effort>` is `low` or `medium`, any other refused unless `--owner-effort` says the owner named it; names the worker `<topic>-<n>-<k>` at the next attempt, or the newest with `--continue`, adds or reuses its worktree `<checkout>/.claude/worktrees/<name>`, opens its tab, starts Claude at that effort, answers the folder-trust dialog, sends it kickoff's `work` prompt with `--leader`, and prints `<name>@<pane> <worktree>` |
