...
1. Work in the issue's worktree, the one `git worktree list` names `*-<issue>`, and `name` yourself after it when you came from another issue. `read` the issue and its parent, and `claim` the issue; held by another session, `comment` `WAITING: held by <branch>` on the issue and stop.
