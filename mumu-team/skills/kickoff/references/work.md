...
1. Work in the task's worktree, the one `git worktree list` names `*-<n>-<k>`. `read` the task and its parent goal, and `claim` the attempt; held by another session, `comment` `BLOCKED: held by <branch>` on the task, `prompt` the leader `see <task-url>`, and stop.
...
While your own reviewer or eval runs, wait in one bounded foreground Bash poll, since `stop.py` blocks every stop.
