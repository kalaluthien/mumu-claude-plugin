...
1. `name` yourself after your checkout's GitHub repository, or `<folder>-lead` as your rules' Folder leads say, unless already so named; `team-watch` starts with this skill, and the Stop hook names the command to arm it again once you hold a root goal and it is not running. Then ask the owner every question at once with `AskUserQuestion`.
