...
A goal's issues live in its leader's repository, the one its checkout clones; work for another project's repository is handed off to that project's leader. The default branch is `gh repo view --json defaultBranchRef -q .defaultBranchRef.name`, and `<hooks>` is `git -C <checkout> rev-parse --path-format=absolute --git-path hooks`.
...
| `handoff` | `gh issue transfer <url> <owner>/<target-repo>`, which prints the new url; then `prompt` the target project's `<project>-lead` in `live` `see <new-url>`, or, none listed, `start-lead` it with `/mumu-team:kickoff <new-url>` |
