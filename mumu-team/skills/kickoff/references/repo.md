...
| `file` | root goal: `gh issue create -R <repo> --title "<title>" --label goal --body-file -`; a goal under a goal: the same with `--parent <goal-url>`; task: `gh label create effort:<effort> -R <repo> --force`, `<effort>` `low` or `medium` unless the owner named another, then the same create with `--parent <goal-url> --label task --label effort:<effort>` |
