...
3. Find the root goals and root tasks you now hold with `gh issue list --state open --search "no:parent-issue label:kind:goal,kind:task" --json number,title,url`, with `--label scope:<folder>` added for a `<folder>-lead`, run in your checkout, and `read` each.
4. `prompt` each worker in `live` whose worktree lies in your checkout `see <its task-url>`, then continue at Lead 4 of [lead-goal.md](lead-goal.md), as [resume.md](resume.md) does.
