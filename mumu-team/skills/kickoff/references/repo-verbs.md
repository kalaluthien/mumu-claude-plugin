...
| `clean` | for each worker `git -C <checkout> worktree remove <worktree>`, `git -C <checkout> branch -D <branch>` and `git -C <checkout> push origin --delete <branch>`, the branch being merged by squash; then `git -C <checkout> pull --ff-only`, and remove each of `<hooks>/pre-commit` and `<hooks>/pre-push` that `cmp -s` finds equal to the guard, as `rm <hooks>/pre-commit` or `rm <hooks>/pre-push` with `<hooks>` written out as its literal absolute path, no variable, so the allow rules `rm */.git/hooks/pre-commit` and `rm */.git/hooks/pre-push` match it; last `tab-sweep.py <checkout>`, which closes every tab whose panes run no agent and sit under `<checkout>/.claude/worktrees/`, so no worker tab outlives its session |
