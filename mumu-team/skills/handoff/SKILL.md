...
4. `herdr agent list` shows that lead: `herdr agent prompt <lead> "see <goal-url>"`. Otherwise `lead-start.py <checkout> <goal-url>`, which starts it as `--agent mumu-team:lead` and prompts it, and a folder lead names itself from the goal's label; a start-up dialog in its new tab is the owner's to answer there.
