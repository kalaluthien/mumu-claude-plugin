...
2. Words that add to a task already open there (`gh issue list -R <repo> --search "<words> label:kind:task"`): `comment` them on it, as said, and go to step 3 with its url. Otherwise `file` a root task in that repository, labelled `kind:task` and `effort:medium`: a title of the request, `## Goal` quoting the owner's words as said, and `## Definition of done` as `- set by <repo>-lead`, since the lead asks the owner for its criteria.
