...
A unit is a widget file or player.html. Fails on:
- a mapping row in SKILL.md naming no file in widgets/;
...
- in skin.css: a missing token kind; a motion token over 200ms; a colour pair under
...
def failures(skill):
    ...
    tokens = dict(DEFINED.findall((skill / "skin.css").read_text()))
    semantic = {t for t in tokens if not t.startswith("--p-")}
    for f in sorted(widgets.glob("*.html")) + [skill / "player.html"]:
