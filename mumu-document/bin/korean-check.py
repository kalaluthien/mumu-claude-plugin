...
f.onload = function () {
  ...
  setTimeout(function () {
    ...
    document.body.dataset.r = JSON.stringify({ lang: d.documentElement.lang, text: out });
  }, 300);
};
...
def rendered(page):
    ...
    with tempfile.NamedTemporaryFile("w", suffix=".html", delete=False) as frame:
        frame.write(FRAME)
    try:
        r = subprocess.run([CHROME, "--headless", "--disable-gpu", "--allow-file-access-from-files", "--dump-dom",
                            "--virtual-time-budget=3000", f"file://{frame.name}#file://{page}"],
                           capture_output=True, text=True, timeout=60)
    finally:
        os.unlink(frame.name)
...
def main():
    ...
    if not page.is_file() or not os.access(CHROME, os.X_OK):
        print(f"korean-check.py: no file {page}" if not page.is_file() else f"korean-check.py: no Chrome at {CHROME}",
              file=sys.stderr)
        return 2
    seen = rendered(page)
    if seen is None:
