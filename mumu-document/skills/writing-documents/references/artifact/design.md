...
The look every Artifact page takes: an editorial magazine, black on white.
[skin.css](skin.css) turns it into tokens; a page follows the skin, and this
file says why. In this plugin:

- Fonts: the open substitutes below, loaded from Google Fonts (Playfair
  Display, Source Serif 4, Inter), with Noto Serif KR and Noto Sans KR for
  Korean; offline, installed faces.
- Headings: `h1` display-md, `h2` display-sm, both display serif at 400;
  `h3` display-xs in the sans. Body is the body serif; controls, captions,
  figure labels and metadata are the sans.
- The accent is ink: it marks focus, hover and the current step. Links take
  `--link`. Borders on controls and figure boxes are the body grey, so they
  pass 3:1 and a hover to ink shows; row dividers are the hairline.
- Figure boxes are square too; a `state-machine` state keeps its rounded
  corners, the sign of a state.
- Dark mode, which the brand does not define, inverts the duet.
- Figures keep the diagram palette and status colours: a figure needs kinds
  and pass/fail, which a magazine page does not.
