...
`AGENTS.md` or `CLAUDE.md` and read now, else a document of the doctype the
question asks for, written by the `writing-documents` skill, loaded first.
...
| more, with the `Artifact` tool in the session | a page of questions in the `#round` form, recommendations prefilled, handed to `show-me` to deliver |
