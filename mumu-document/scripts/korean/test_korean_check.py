...
CHECK = ROOT / "scripts" / "korean" / "korean-check.py"
...
class KoreanCheck(unittest.TestCase):
    ...
    def test_sentence_heading_fails(self):
        for heading in ("작업 큐는 파일 세 개예요", "큐가 일을 받습니다", "큐가 일을 받는다", "큐의 구조."):
            code, out = run(GOOD.replace("<h1>작업 큐의 구조</h1>", f"<h1>{heading}</h1>"))
            self.assertEqual(code, 1, heading)
            self.assertIn(f"sentence heading: {heading}", out)
