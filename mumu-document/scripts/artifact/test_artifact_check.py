...
class LabelGeometry(unittest.TestCase):
    ...
    def test_overlapping_labels_fail(self):
        ...
        self.assertIn("labels 2 hints 0/0 FAIL", out)

    def test_label_outside_its_figure_fails(self):
        code, out = run(PAGE.replace('x="160"', 'x="250"'))
        self.assertEqual(code, 1)
        self.assertIn("labels 1 hints 0/0 FAIL", out)


HINT = """<!doctype html><html lang="ko"><meta charset="utf-8"><title>힌트</title>
<main><p><button popovertarget="h1">큐</button>에 일을 넣어요.</p><div id="h1" popover>일을 차례로 담아 두는 곳이에요.</div>
<p><button popovertarget="h2">작업자</button>가 일을 꺼내요.</p><div id="h2" popover>일을 하나씩 처리해요.</div></main></html>"""


@unittest.skipUnless(CHROME.exists(), "no Chrome")
class HintTap(unittest.TestCase):
    def test_each_hint_opens_on_tap(self):
        code, out = run(HINT)
        self.assertEqual(code, 0, out)
        self.assertIn("hints 2/2", out)

    def test_hint_that_does_not_open_fails(self):
        code, out = run(HINT.replace('<div id="h2" popover>', '<div id="h2">'))
        self.assertEqual(code, 1)
        self.assertIn("hints 1/2", out)
