...
# `... labels <n> hints <opened>/<total> pass|FAIL`, then `pass` or `FAIL`. A run fails on a sideways scroll, text
# under 11 px (Hangul or Han under 12 px), an SVG label overlapping another or leaving its
# figure (n counts them), a hint a tap leaves shut, or an uncaught error; the reduced run also on a running animation or
...
f.onload = function () {
  ...
  setTimeout(function () {
    ...
    var ok = e.scrollWidth == e.clientWidth && s[0] >= 11 && s[1] >= 12 && bad == 0 && opened == hints.length && (!reduced || (anim == 0 && shown == total));
    document.body.dataset.r = (reduced ? 'reduced ' : 'motion ') + e.scrollWidth + '/' + e.clientWidth + ' ' + s[0] + 'px ' + s[1] + 'px anim ' + anim + ' steps ' + shown + '/' + total + ' labels ' + bad + ' hints ' + opened + '/' + hints.length + (ok ? ' pass' : ' FAIL');
  }, 500);
};
