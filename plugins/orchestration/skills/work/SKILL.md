---
name: work
description: Use when this session is a worker given an issue URL ("work <issue-url> leader <pane>") - claim its branch, land one PR, get it reviewed and merged. Not for splitting a goal into issues (lead).
---
...
7. Blocked on a decision that is not yours: comment `BLOCKED: <question>` on the issue, then prompt the leader's pane (`github.md`). The leader's pane is gone from `herdr agent list`: stop after posting.
