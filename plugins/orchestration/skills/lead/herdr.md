...
| brief it | `herdr agent prompt <pane> "work <issue-url> leader $HERDR_PANE_ID"` (never on the start line) |
...
| relaunch | new pane in the same worktree, start with `-- --continue --model opus --effort <e>`, prompt the same brief |
